...
def get_table_column_count(schema_name: str, table_name: str):
    sql_str = f"select count(*) as row_num from INFORMATION_SCHEMA.COLUMNS where TABLE_SCHEMA='{schema_name}' and TABLE_NAME='{table_name}'"
    print(sql_str)
    cursor.execute(sql_str)
    column_count = cursor.fetchone()
    print('column_count', column_count)
    return column_count['row_num']


# return null or '' or ' ' value count for a column
def column_value_nullOrEmpty(schema_name: str, table_name: str, column_name: str):
    sql_str = f"select count(*) as null_count from {schema_name}.{table_name}" \
    ...
    return invalid_column_value_count['null_count']
...
def verify_no_duplicate(schema_name: str, table_name: str, column_name: str):
    sql_str = f"select count({column_name}) as all_count from {schema_name}.{table_name}"
    sql_str_dis = f"select count(distinct {column_name}) as dis_count from {schema_name}.{table_name}"
    count_one = cursor.execute(sql_str).fetchone()
    count_two = cursor.execute(sql_str_dis).fetchone()
    return count_one['all_count'] - count_two['dis_count']
...
def verify_special_businessRule(schema_name: str, table_name: str, column_name: str, expect_per: float):
    sql_str_one = f"select count(distinct {column_name}) as neg_count from {schema_name}.{table_name} group by customerId" \
                  f" having sum(cast(replace(amount,'$','') as int))<0;"
    records_amount_with_negative_values = cursor.execute(sql_str_one).fetchone()[0]
    sql_str_two = f"select count(distinct {column_name}) as all_count from {schema_name}.{table_name}"
    tot_records = cursor.execute(sql_str_two).fetchone()[0]

    return get_result_by_percentage(records_amount_with_negative_values['neg_count'], tot_records['all_count'], expect_per,
